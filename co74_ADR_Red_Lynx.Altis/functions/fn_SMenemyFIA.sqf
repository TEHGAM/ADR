...
for "_x" from 0 to (2 + (random 4)) do {
	...
	_infteamPatrol = [_randomPos, WEST, (configfile >> "CfgGroups" >> "West" >> "Guerilla" >> "Infantry" >> [INF_TEAMS] call BIS_fnc_selectRandom)] call BIS_fnc_spawnGroup;
...
for "_x" from 0 to 2 do {
	_IRGsniperGroup = createGroup WEST;
	_randomPos = [getPos sideObj, 600, 100, 20] call BIS_fnc_findOverwatch;
	_IRGsniperGroup = [_randomPos, WEST, (configfile >> "CfgGroups" >> "West" >> "Guerilla" >> "Infantry" >> "IRG_SniperTeam_M")] call BIS_fnc_spawnGroup;
