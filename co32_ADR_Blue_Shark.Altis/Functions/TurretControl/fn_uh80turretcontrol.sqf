...
if (_v getVariable "turretL_locked") then {
	turretLUnlockAction = player addAction [
		"Unlock Turret - Left",
		AW_fnc_uh80TurretActions,
		[_v,1,0],
		80,
		FALSE,
		FALSE,
		'',
		'[] call AW_fnc_conditionUH80TurretActionUnlockL'
	];
...
if (_v getVariable "turretR_locked") then {
	turretRUnlockAction = player addAction [
		...
		FALSE,
		'',
		'[] call AW_fnc_conditionUH80TurretActionUnlockR'
	];
...
if (!(_v getVariable "turretL_locked")) then {
	turretLLockAction = player addAction [
		...
		FALSE,
		'',
		'[] call AW_fnc_conditionUH80TurretActionLockL'
	];
...
if (!(_v getVariable "turretR_locked")) then {
	turretRLockAction = player addAction [
		...
		FALSE,
		'',
		'[] call AW_fnc_conditionUH80TurretActionLockR'
	];
	...
	[_v] spawn {
		...
		while {uh80turret_control} do {
			...
			if ((!alive player) || {(player != player)} || {(_v2 != _v)}) then {
				...
				[_v,1,0] call AW_fnc_uh80TurretReset;
				[_v,2,0] call AW_fnc_uh80TurretReset;
			};
			sleep 3;
		};
