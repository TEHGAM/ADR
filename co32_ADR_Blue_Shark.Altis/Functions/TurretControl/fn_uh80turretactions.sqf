...
if (_lock == 0) exitWith {
	if (_turret == 1) then {
		[[_v,1,"LMG_Minigun_Transport",1],"AW_fnc_uh80Turret",true,false] spawn BIS_fnc_MP;
		_v setVariable ["turretL_locked",FALSE,TRUE];
	};
	if (_turret == 2) then {
		[[_v,2,"LMG_Minigun_Transport2",1],"AW_fnc_uh80Turret",true,false] spawn BIS_fnc_MP;
		_v setVariable ["turretR_locked",FALSE,TRUE];
	};	
...
if (_lock == 1) exitWith {
	if (_turret == 1) then {
		[[_v,1,"LMG_Minigun_Transport",0],"AW_fnc_uh80Turret",true,false] spawn BIS_fnc_MP;
		_v setVariable ["turretL_locked",TRUE,TRUE];
	};
	if (_turret == 2) then {
		[[_v,2,"LMG_Minigun_Transport2",0],"AW_fnc_uh80Turret",true,false] spawn BIS_fnc_MP;
