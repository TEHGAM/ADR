...
//-------------------------------------------------------------------------- CREATE GROUP, VEHICLE AND UNIT
	...
	[(units _aGroup)] call AW_fnc_setSkill2;
	[(units _bGroup)] call AW_fnc_setSkill2;
	[(units _cGroup)] call AW_fnc_setSkill2;
...
//--------------------------------------------------------------------------- ADD ACTION TO OBJECTIVE. NOTE: NEEDS WORK STILL. avoid BIS_fnc_MP! Good enough for now though.
	...
	sleep 0.1;
	[_intelObj,"AW_fnc_addActionSurrender",nil,true] spawn BIS_fnc_MP;
	sleep 0.1;
...
while { sideMissionUp } do {
	...
	if (GETTING_AWAY) then {
		...
		if (HE_SURRENDERS) then {
			...
			[_intelObj,"AW_fnc_removeAction1",nil,true] spawn BIS_fnc_MP;
		};
