...
if (_type in _ghosthawk) then {
	...
	_unit addAction ["<t color='#00B2EE'>  вЂў РЈРґР°Р»РёС‚СЊ СЃРЅР°СЂСЏР¶РµРЅРёРµ</t>", "scripts\vehicle\drop\drop.sqf",[1],0,false,true,"","driver _target == _this"];
...
if (_type in _VASable) then { 
	_unit addAction ["<t color='#ff1111'>РњРѕР±РёР»СЊРЅС‹Р№ VAS</t>","scripts\VAS\open.sqf",[],10,true,true,'((vehicle player) == player) && ((player distance _target) < 5)'];
};
...
_unit addAction ["<t color='#00B2EE'>  вЂў РЈРґР°Р»РёС‚СЊ СЃРЅР°СЂСЏР¶РµРЅРёРµ</t>","scripts\vehicle\clear\clear.sqf",[],-97,false];