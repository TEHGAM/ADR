...
#define INFANTRY_STATIC "O_HMG_01_F","O_HMG_01_high_F","O_HMG_01_A_F","O_GMG_01_F","O_GMG_01_high_F","O_GMG_01_A_F"
...
while {!_accepted} do {
    ...
    if (_distance > 3000) then {
    	_distance = [_flatPos, getMarkerPos "sideMarker"] call BIS_fnc_distance2D;
...
{ _x setMarkerPos _flatPos; } forEach ["infantryMarker", "infantryCircle"];
infantryMarkerText = " Прерванный полёт"; publicVariable "infantryMarkerText";
"infantryMarker" setMarkerText "Спецоперация: Прерванный полёт (без техники)"; publicVariable "infantryMarker";
...
_heliObj = createVehicle ["B_Heli_Attack_01_F", [300,300,300], [], 0, "NONE"];  
...
for "_x" from 1 to 6 do {
    ...
    [_patrolGroup, (getMarkerPos "infantryMarker"), 200] call BIS_fnc_taskPatrol;
...
for "_x" from 1 to 10 do {      
    ...
    _houseUnit = ([INFANTRY_HOUSE] call BIS_fnc_selectRandom) createUnit [_randomPos,_houseGroup];
    doStop _houseUnit;
};
...
showNotification = ["NewInfantryMission", "Прерванный полёт"]; publicVariable "showNotification";

infantryMissionUp = true; publicVariable "infantryMissionUp";
INF_SUCCESS_SABOTAGE = false; publicVariable "INF_SUCCESS_SABOTAGE";
INF_SUCCESS_GETDATA = false; publicVariable "INF_SUCCESS_GETDATA";
_showSabotageMessage = true;
_showGetdataMessage = true;
while { infantryMissionUp } do {
    sleep 2;

	// Heli part done
    if ((INF_SUCCESS_SABOTAGE || !alive _heliObj) && _showSabotageMessage) then {
    	hint "";    
    	_showSabotageMessage = false;	     	   
        if (INF_SUCCESS_GETDATA) then {
    ...
    // Scout box part done
    if (INF_SUCCESS_GETDATA && _showGetdataMessage) then {
        hint "";            
        _showGetdataMessage = false;        
        if (INF_SUCCESS_SABOTAGE) then {
	...
	if (INF_SUCCESS_SABOTAGE && INF_SUCCESS_GETDATA) exitWith {		
		infantryMissionUp = false; publicVariable "infantryMissionUp";        
		{ _x setMarkerPos [-12000,-12000,-12000]; publicVariable _x; } forEach ["infantryMarker", "infantryCircle"];
        "infantryMarker" setMarkerText ""; publicVariable "infantryMarker";
		[] call QS_fnc_InfantryhintSUCCESS;	
        sleep 120;
