...
//if(((_unit modelToWorld [0,0,_minY]) select 2) < 0) exitWith {hint "Слишком низко для сброса груза."};
if((((getPos _unit select 2) + minLiftingHeight) + _minY)  < 0) exitWith {hint "Слишком низко для сброса груза."};
