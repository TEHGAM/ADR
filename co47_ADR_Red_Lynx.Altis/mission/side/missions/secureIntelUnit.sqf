...
#define OBJVEH_TYPES "B_MRAP_01_F", "I_MRAP_03_F", "C_Offroad_01_F", "C_SUV_01_F", "C_Van_01_transport_F", "B_Heli_Light_01_armed_F"
#define ALTVEH_TYPES "B_MRAP_01_F"
#define OBJUNIT_TYPES "B_officer_F", "I_officer_F", "I_Soldier_SL_F", "B_Soldier_SL_F", "B_recon_TL_F", "B_diver_TL_F"
...
while {!_accepted} do {
	...
	if ((_flatPos distance (getMarkerPos "respawn_east")) > 3000 && (_flatPos distance (getMarkerPos currentAO)) > 3000) then {
		_accepted = true;
	};
};
...
_aGroup = createGroup WEST;
_bGroup = createGroup WEST;
_cGroup = createGroup WEST;
_surrenderGroup = createGroup EAST;
...
"B_crew_F" createUnit [_flatPos1, _aGroup];
...
"B_crew_F" createUnit [_flatPos1, _bGroup];
...
"B_crew_F" createUnit [_flatPos1, _cGroup];
sleep 0.3;
...
_targetTrigger setTriggerActivation ["EAST", "PRESENT", false];
...
//--------------------------------------------------------------------------- SPAWN GUARDS
_enemiesArray = [_obj1] call QS_fnc_SMenemyWESTintel;
...
while { sideMissionUp } do {
	...
	if (NOT_ESCAPING) then {
		...
		if (_intelObj call BIS_fnc_enemyDetected) then {
			...
			hqSideChat = "Вас обнаружили; противник предпринимает попытку уйти вместе с разведданными!"; publicVariable "hqSideChat"; [EAST, "HQ"] sideChat hqSideChat;
	...
	if (HE_ESCAPED) exitWith {
			//---------- DE-BRIEF
			hqSideChat = "Цель ушла от преследования! ЗАДАНИЕ ПРОВАЛЕНО!"; publicVariable "hqSideChat"; [EAST, "HQ"] sideChat hqSideChat;
			sideMissionUp = false; publicVariable "sideMissionUp";
			[] spawn QS_fnc_SMhintFAIL;
			{ _x setMarkerPos [-10000, -10000, -10000]; } forEach ["sideMarker", "sideCircle"]; publicVariable "sideMarker";
	...
	if (SM_SUCCESS) exitWith {
		sleep 0.3;
		//---------- DE-BRIEF
		hqSideChat = "Разведданные захвачены. ЗАДАНИЕ ВЫПОЛНЕНО! Данные отправлены на исследование."; publicVariable "hqSideChat"; [EAST, "HQ"] sideChat hqSideChat;
		sideMissionUp = false; publicVariable "sideMissionUp";
