...
#define INFANTRY_HOUSE "B_Soldier_AR_F","B_soldier_M_F","B_Soldier_TL_F"
#define INFANTRY_MOTORIZED "B_G_Offroad_01_armed_F"
...
for "_x" from 1 to 6 do {
    ...
    _patrolGroup = [_randomPos, WEST, (configfile >> "CfgGroups" >> "WEST" >> "BLU_F" >> "Infantry" >> [INFANTRY_PATROL] call BIS_fnc_selectRandom)] call BIS_fnc_spawnGroup;
...
// spawn technical
for "_x" from 1 to 2 do {
    _technicalGroup = createGroup WEST;
    if((count _nearRoads) > 0) then {
        _roadSegment = _nearRoads call BIS_fnc_selectRandom;
        _randomPos = getPos _roadSegment;
        _direction = getDir _roadSegment;
    ...
    "B_engineer_F" createUnit [_randomPos,_technicalGroup];
    "B_engineer_F" createUnit [_randomPos,_technicalGroup];
...
while { sideMissionUp } do {
    ...
    if ((SM_SUCCESS_SABOTAGE || !alive _heliObj) && _showSabotageMessage) then {
    	...
    	publicVariable "hqSideChat"; [EAST,"HQ"] sideChat hqSideChat;		
    };
