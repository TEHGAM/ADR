...
while {!_accepted} do {
	...
	if ((_flatPos distance (getMarkerPos "respawn_east")) > 2000 && (_flatPos distance (getMarkerPos currentAO)) > 800) then {
		_accepted = true;
	};
};
...
priorityObj1 = "B_MBT_01_arty_F" createVehicle _flatPos1;
waitUntil {!isNull priorityObj1};
priorityObj1 setDir _PTdir;

sleep 0.3;

priorityObj2 = "B_MBT_01_arty_F" createVehicle _flatPos2;
...
ammoTruck = "B_Truck_01_ammo_F" createVehicle _flatPos3;
...
"B_officer_F" createUnit [_flatPos, _priorityGroup];
"B_engineer_F" createUnit [_flatPos, _priorityGroup];
"B_engineer_F" createUnit [_flatPos, _priorityGroup];
...
_enemiesArray = [priorityObj1] call QS_fnc_PTenemyWEST;
...
while { canMove priorityObj1 || canMove priorityObj2 } do {
	...
	while {!_accepted} do {
		...
		if (!isNull _unit) then {
			...
			if ((_targetPos distance (getMarkerPos "respawn_east")) > 1000 && vehicle _unit == _unit && side _unit == EAST) then { 
				_accepted = true; 
			} else {
				sleep 7;															// default 10
			};
	...
	if (PARAMS_ArtilleryTargetTickWarning == 1) then {
		hqSideChat = _firingMessages call BIS_fnc_selectRandom; publicVariable "hqSideChat"; [EAST,"HQ"] sideChat hqSideChat;
	};
