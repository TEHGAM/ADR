...
while {!_accepted} do {
	...
	if ((_flatPos distance (getMarkerPos "respawn_east")) > 1000 && (_flatPos distance (getMarkerPos currentAO)) > 500) then {
		_accepted = true;
	};
};
...
_enemiesArray = [sideObj] call QS_fnc_SMenemyWEST;
...
while { sideMissionUp } do {
	if (!alive sideObj) exitWith {
		//-------------------- DE-BRIEFING
		hqSideChat = "Данные о прототипе утеряны. Задание провалено!"; publicVariable "hqSideChat"; [EAST,"HQ"] sideChat hqSideChat;
