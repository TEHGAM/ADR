...
if ((count enemyCasArray) < _jetLimit) then {
	...
	if (isNull enemyCasGroup) then {enemyCasGroup = createGroup WEST;};

	_jetPilot = enemyCasGroup createUnit ["B_Pilot_F", [0, 0, (1000 + (random 1000))], [], 0, "NONE"];
