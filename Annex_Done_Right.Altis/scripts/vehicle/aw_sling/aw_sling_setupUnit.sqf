...
if(isNil "aw_sling_cameraOn") then {aw_sling_cameraOn = false};
 ...
 _vehicle addAction ["<t color='#d63333'>" + "Подцепить" + "</t>","scripts\vehicle\aw_sling\aw_sling_attach.sqf",[],21,true,true,"","(_this == driver _target) AND (count ((getPos _target) nearEntities [['Car','Motorcycle','Tank','ship'],5]) > 0) AND !(_target getVariable 'aw_sling_attached')"];
 _vehicle addAction ["<t color='#d63333'>" + "Отцепить" + "</t>","scripts\vehicle\aw_sling\aw_sling_detach.sqf",[],21,true,true,"","(_this == driver _target) AND (_target getVariable 'aw_sling_attached')"];
_vehicle addAction ["<t color='#d63333'>" + "Камера (вкл/выкл)" + "</t>","scripts\vehicle\aw_sling\aw_sling_toggleDisplay.sqf",[],-98,false,true,"","(_this == driver _target)"];
//pilot eject
