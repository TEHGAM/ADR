...
_camera = _helo getVariable "aw_sling_camera";
while{alive _helo} do 
{
	waitUntil{sleep 0.5;(player == driver _helo) AND aw_sling_cameraOn};
	...
	waitUntil{!(alive _helo) OR !(player == driver _helo) OR !(alive player) OR !aw_sling_cameraOn};
	_disp = uiNamespace getVariable "BIS_fnc_PIP_RscPIP";
