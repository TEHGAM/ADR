...
/* Конфликтует с VAS и Арсеналом (загрузка по умолчанию)
  _unit = _this select 0;
...
BTC_move_to_mobile =
{
  ...
  if (speed _mobile > 5) exitWith {hint "Переместится нельзя пока КШМ в пути!";};
  _pos = getPos _mobile;
...
BTC_out_of_lifes =
{
  if (BTC_lifes == 0) then
  {
    ...
    player enableSimulation false;
    titleText ["Ваши жизни исчерпаны", "BLACK FADED"];//BLACK FADED
    sleep 1;
    if (BTC_spectating == 0) then
    {
      while {true} do
      {
        ...
        player setpos [0,0,6000];
        player setVelocity [0,0,0];
        titleText ["Ваши жизни исчерпаны", "BLACK FADED"];
        sleep 1;
      };
...
BTC_r_create_dialog_mobile =
{
  if (BTC_active_lifes != 1 || BTC_lifes != 0) then
  {
    ...
    while {!BTC_r_chosen} do
    {
      ...
      //player setVelocity [0,0,0];
      call BTC_r_load;
      if (count BTC_r_list == 0) then {titleText ["Нет точки для возрождения.", "PLAIN"];};
      sleep 1;
    };
    player allowdamage true;
    closeDialog 0;
...
BTC_r_s_load =
{
  ...
  if (count _list_name == 0) then {titleText ["Не найдено унитов!","PLAIN DOWN"];BTC_r_s_target = BTC_r_base_spawn;} else {titleText ["","PLAIN DOWN"];};
  true
};
